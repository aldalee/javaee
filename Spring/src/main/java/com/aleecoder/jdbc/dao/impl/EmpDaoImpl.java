...
public class EmpDaoImpl implements EmpDao {
    ...
    public int findEmpCount() {
        // queryForObject 传入两个参数：SQL语句、返回值类型
    ...
    public Emp findByEmpno(int empno) {
        ...
        return jdbcTemplate.queryForObject("select * from emp where empno =?", rowMapper, empno);
    }
    ...
    public List<Emp> findByDeptno(int deptno) {
         ...
         * */
        BeanPropertyRowMapper<Emp> rowMapper = new BeanPropertyRowMapper<>(Emp.class);
        return jdbcTemplate.query("select * from emp where deptno =?", rowMapper, deptno);
    }
